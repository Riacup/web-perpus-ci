...
class DaftarPengunjung extends MY_backend
{
    function __construct()
    {
		...
		$GLOBALS['folder_foto_pengunjung'] = 'public/daftarpengunjung_foto_pengunjung/';
		...
		$this->breadcrumb->add_crumb('Daftar Pengunjung');
		
		$this->data['primary_title'] = '<i class="fa fa fa-gear"></i> Daftar Pengunjung';
		$this->data['sub_primary_title'] = '';
		
		$this->data['sub_title'] = 'Daftar Pengunjung';
		$this->layout->set_title($this->data['sub_title']);
		
		$this->table_daftarpengunjung = 'daftarpengunjung'; 
	...
	function show(){
		...
		$this->data['list'] = $this->wd_db->get_data_row($this->table_daftarpengunjung,array('id'=>$id));
	...
	public function validation_rule(){
		$this->data['rules'] = array(
			array('field'   => 'nama_pengunjung', 'label' => 'Nama_pengunjung','rules'   => 'required'),
			array('field'   => 'tgl_daftar', 'label' => 'Tgl_daftar','rules'   => 'required'),
		);
	...
	function save_action(){
		...
		$this->wd_db->add_dml_get_id($this->table_daftarpengunjung,$data);
	...
	function update_action(){
		...
		$old = $this->wd_db->get_data($this->table_daftarpengunjung,array('id' => $id)) ;
		...
		$this->wd_db->edit_dml($this->table_daftarpengunjung,$data,$where);
	...
	function delete_action(){
		...
		if($this->input->get('confirm') == null){
			$this->confirm_delete($this->table_daftarpengunjung,'id','nama_pengunjung');
			return;
		}
		...
		$this->wd_db->del_dml_where_in($this->table_daftarpengunjung,'id',$del_id);
	...
	public function dataTable() {
		$this -> load -> library('Datatable', array('model' => 'm_datatables', 'rowIdCol' => 'daftarpengunjung.id'));
		$jsonArray = $this -> datatable -> datatableJson(array());


		foreach ($jsonArray['data'] as $index => $json) {
			$jsonArray['data'][$index]['daftarpengunjung']['tgl_daftar']=convertDate($jsonArray['data'][$index]['daftarpengunjung']['tgl_daftar'], 'd m y');
...
/* End of file DaftarPengunjung.php */
/* Location: ./application/modules/DaftarPengunjung/controllers/DaftarPengunjung.php */
/* Please DO NOT modify this information : */
/* Generated by IndonesiaIT Codeigniter CRUD Generator 2019-07-02 08:25:13 */
/* indonesiait.com */